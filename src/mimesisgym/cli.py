...
def _provider(args: argparse.Namespace):
    ...
    return ResponsesProvider(client, args.model, reasoning_effort=args.reasoning)
...
def _eval(args: argparse.Namespace) -> None:
    ...
    runner = EvalRunner(DockerBackend(), _provider(args), config)
...
def _parser() -> argparse.ArgumentParser:
    ...
    evaluate.add_argument("--max-tool-calls", type=int, default=40)
    evaluate.add_argument("--max-output-tokens", type=int, default=6000)
    evaluate.add_argument("--max-total-output-tokens", type=int, default=24000)
